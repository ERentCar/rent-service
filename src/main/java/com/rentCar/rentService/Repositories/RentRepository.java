...
public interface RentRepository extends JpaRepository<Rent,Long> {
    @Query(value="select R.*from rents as R left join reservations RV " +
            "on R.reservation_id=RV.id where RV.client_id=?1 group by R.id; ", nativeQuery = true)
    List<Rent> getRentsByClientId(Long clientId);

    @Query(value="select R.*from rents as R left join reservations RV " +
            "on R.reservation_id=RV.id where RV.owner_id=?1 group by R.id; ", nativeQuery = true)
